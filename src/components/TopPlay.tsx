...
import { TopChartCardProps } from "../interfaces/components";
...
const TopChartCard = ({ song, i, isPlaying, activeSong, handlePauseClick, handlePlayClick }: TopChartCardProps) => (
  <div className={`w-full flex flex-row items-center hover:bg-[#be4d4d] ${activeSong?.title === song?.title ? "bg-[#751b1b]" : "bg-transparent"} py-2 p-4 rounded-lg cursor-pointer mb-2`}>
...
const TopPlay = () => {
  ...
  const { data } = useGetTopChartsUSQuery("");
    ...
    <div ref={divRef} className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col">
      <div className="w-full flex flex-col">
        ...
        <div className="mt-4 flex flex-col gap-1">
          {topPlays?.map((song: Track, i: number) => (
            <TopChartCard
              key={song.key}
              song={song}
              i={i}
